...
module.exports = sock = async (sock, m, msg, chatUpdate, store = null) => {
	...
	try {
		...
		if (isGroupBlacklist) {
			// if it's not a command, ignore completely
			if (!isCmd) return;
			// allow only the unblacklist command to be processed in blacklisted groups
			const allowed = ['deleteblacklist'];
			if (!allowed.includes(command)) return; // silently ignore
		};

		// Interactive flow: if user requested a list of groups and replies with a number, handle selection
		...
		switch (command) {
			...
			case 'deleteblacklist': {
				// remove current chat from blacklist
...
break;
			...
			case 'menu': {
				try {
					...
					let samplemenu = `sᴇʟᴀᴍᴀᴛ ᴅᴀᴛᴀɴɢ ᴅɪ ᴍᴀᴏᴄʜᴀɴ ᴍᴅ 👋 

ᴀᴘᴀ ɪᴛᴜ ᴍᴀᴏᴄʜᴀɴ ᴍᴅ 🤷‍♂️? ᴍᴀᴏᴄʜᴀɴ ᴍᴅ ᴀᴅᴀʟᴀʜ sᴇʙᴜᴀʜ ᴀsɪsᴛᴇɴ ʙᴏᴛ✨ ʏᴀɴɢ ᴅᴀᴘᴀᴛ ᴍᴇᴍʙᴜᴀᴛ ᴍᴜ ᴛᴇʀʜɪʙᴜʀ, ᴀᴋᴜ ᴀᴅᴀʟᴀʜ ᴛᴇᴍᴀɴ ᴍᴜ ʟᴏʜ 🤗
┏一一〖 𝑰𝒏𝒇𝒐 𝑼𝒔𝒆𝒓 〗一玄⪩
┃ ➣ ✇ ɴᴀᴍᴇ: *${m.pushname}* 👋
...
`;
			...
			case 'allmenu': {
				...
				let me = m.sender;
				await m.react('⏱️');
				let simplemenu = `sᴇʟᴀᴍᴀᴛ ᴅᴀᴛᴀɴɢ ᴅɪ ᴍᴀᴏᴄʜᴀɴ ᴍᴅ 👋 
