...
let lastGroupList = {}; // temp storage for interactive group selection per user
...
module.exports = sock = async (sock, m, msg, chatUpdate, store) => {
	...
	try {
		...
		// normalize bot id for reliable comparison
		...
		if (m.isGroup) {
			m.metadata = await sock.groupMetadata(m.chat).catch(e => null)
			if (m.metadata) {
		...
		if (!m.sender.startsWith(`${antiforeignnumber}`) && db.data.chats[m.chat].antiforeignnum === true) { 
			if (isSuperAdmin || isCreator || isAdmins || !isBotAdmins) return;
		...
		if (db.data.chats[m.chat].antimedia && isMedia) {
			if (isSuperAdmin || isCreator || isAdmins || !isBotAdmins){
			} else {
		...
		if (db.data.chats[m.chat].antilinkgc) {
			if (budy.match(`chat.whatsapp.com`)) {
				if (isSuperAdmin || isAdmins || m.key.fromMe || isCreator) return m.react('❤️');
				await sock.sendMessage(m.chat, {
					delete: {
						remoteJid: m.chat,
						fromMe: false,
		...
		if (isBlacklist) {
			if (isSuperAdmin || isAdmins || isCreator || !isBotAdmins) return;
		...
		// If group is blacklisted: ignore all messages (no replies) except the dedicated unblacklist command
		if (isGroupBlacklist) {
			if (!isCmd) return;
			const allowed = ['deleteblacklist'];
			if (!allowed.includes(command)) return;
		};

		// Interactive flow: handle numeric reply to select group from last list
		if (!isCmd && lastGroupList[m.sender] && /^[0-9]+$/.test(body)) {
			const idx = parseInt(body) - 1;
			const list = lastGroupList[m.sender] || [];
			if (!list[idx]) return newReply('Nomor tidak valid. Coba lagi atau gunakan `cancel`.');
			const groupId = list[idx];
			if (groupblacklist.includes(groupId)) return newReply('Grup sudah ada di blacklist.');
			groupblacklist.push(groupId);
			fs.writeFileSync('./src/data/function/groupblacklist.json', JSON.stringify(groupblacklist, null, 2));
			delete lastGroupList[m.sender];
			newReply(`✅ Grup berhasil ditambahkan ke blacklist: ${groupId}`);
			return;
		}
		...
		switch (command) {
			...
			case 'tebakaplikasi': {
				...
				if (!isPremium && db.data.users[m.sender].limit < 1) return newReply(mess.limit);
				try {
					let result = await fetchAndPick('https://www.sock.my.id/cdn/game/tebakaplikasi.json');
					if (!result || !result.jawaban) return newReply('*Gagal memuat database permainan (kosong atau tidak valid).* ☹️');
					console.log("Jawaban: " + result.jawaban);
					tebakaplikasi[m.chat] = [
						await sock.sendMessage(m.chat, { image: { url: result.image }, caption: `Gambar diatas adalah aplikasi?\n\nWaktu : ${(120000 / 1000).toFixed(2)} detik\n\n_Ketik .nyerah Untuk Menyerah..._\n_Ketik .bantuan Untuk Petunjuk..._` }, { quoted: m }),
						result,
						250,
						setTimeout(() => {
			...
			case 'tebakkata': {
				...
				if (!isPremium && db.data.users[m.sender].limit < 1) return newReply(mess.limit);
				try {
					let result = await fetchAndPick('https://raw.githubusercontent.com/BochilTeam/database/master/games/tebakkata.json');
					if (!result || !result.jawaban) return newReply('*Gagal memuat database permainan (kosong atau tidak valid).* ☹️');
					console.log("Jawaban: " + result.jawaban);
					tebakkata[m.chat] = [
			...
			case 'listmygroups': {
				...
				for (let i = 0; i < groups.length; i++) {
					const g = groups[i];
					ids.push(g.id);
					teks += `${i + 1}. ${g.subject || 'Unknown'}\nID: ${g.id}\n\n`;
				}
				lastGroupList[m.sender] = ids;
				newReply(teks + 'Reply dengan nomor grup untuk menambahkan ke blacklist, atau kirim `cancel` untuk membatalkan.');
			}
			break;

			case 'deleteblacklist': {
				const groupId = m.isGroup ? m.chat : text;
				if (!groupId) return newReply(`Gunakan ${prefix + command} di dalam grup atau sertakan ID grup.`);
				const idx = groupblacklist.indexOf(groupId);
				if (idx === -1) return newReply('Grup tidak ada di blacklist.');
			...
			case 'menu': {
				try {
					...
					let samplemenu = `sᴇʟᴀᴍᴀᴛ ᴅᴀᴛᴀɴɢ ᴅɪ ᴏʟɪɴᴇ ᴍᴅ 👋 

ᴀᴘᴀ ɪᴛᴜ ᴏʟɪɴᴇ ᴍᴅ 🤷‍♂️? ᴏʟɪɴᴇ ᴍᴅ ᴀᴅᴀʟᴀʜ sᴇʙᴜᴀʜ ᴀsɪsᴛᴇɴ ʙᴏᴛ✨ ʏᴀɴɢ ᴅᴀᴘᴀᴛ ᴍᴇᴍʙᴜᴀᴛ ᴍᴜ ᴛᴇʀʜɪʙᴜʀ, ᴀᴋᴜ ᴀᴅᴀʟᴀʜ ᴛᴇᴍᴀɴ ᴍᴜ ʟᴏʜ 🤗
┏一一〖 𝑰𝒏𝒇𝒐 𝑼𝒔𝒆𝒓 〗一玄⪩
┃ ➣ ✇ ɴᴀᴍᴇ: *${m.pushname}* 👋
...
`;
			...
			case 'allmenu': {
				...
				let me = m.sender;
				await m.react('⏱️');
				let simplemenu = `sᴇʟᴀᴍᴀᴛ ᴅᴀᴛᴀɴɢ ᴅɪ ᴏʟɪɴᴇ ᴍᴅ 👋 
...
┗一一玄⪩`;
			...
			if (budy.startsWith('$')) {
				if (!isCreator) return m.react('⚠️');
				if (isBot) return m.react('⚠️');
				logCommand('exec_command', m.sender, budy.slice(2), 'EXEC');
				await m.react('⏱️');
